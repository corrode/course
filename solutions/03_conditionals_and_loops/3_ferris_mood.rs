/// Ferris the crab has moods. Decide which one based on how hungry he is (a
/// `0..=10` scale) and how many naps he's had today.
///
...
///
/// One `if`/`else if`/`else` chain, returning a `&'static str`. The order of
/// the branches matters; the tests will catch you if you get it wrong.
...
fn hangry_overrides_everything() {
    // Ferris is too hungry to care about anything else. If you check naps
    // before hunger, these will fail.
