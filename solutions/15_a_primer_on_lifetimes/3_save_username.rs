...
// Cloning the matching username leaves the caller's records unchanged. The
// returned String has its own buffer, so dropping the records won't free it.
// Returning &str avoids the copy if the records stay alive while we use the
// name.
