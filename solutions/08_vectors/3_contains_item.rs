...
///
/// A read-only operation again, but this time we have to compare each element
/// against `item`.
///
/// Heads-up: you'll want to reach for `Vec::contains`, but its signature is
/// `fn contains(&self, x: &T) -> bool`, and here that's `&String`, while we
