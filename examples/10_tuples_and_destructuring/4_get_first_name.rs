...
///
/// This signature moves the tuple in (it contains `String`s, which aren't `Copy`).
/// After the call, the original `full_name` is no longer valid; try using it after calling this function and read the error.
/// `swap_values` below takes `(i32, i32)`, which is `Copy`, so the original is still usable.
/// The moves chapter covers this distinction.
///
