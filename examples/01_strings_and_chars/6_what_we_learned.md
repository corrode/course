...
- `&str` is a borrowed view into UTF-8 text; `String` is an owned, growable buffer; `char` is one Unicode scalar value.
  Functions that read take `&str`, functions that produce return `String`.
- `str::len` is byte length, not character count.
  Use `s.chars().count()` when you mean characters.
- `str::chars()` returns an iterator.
  Anything that takes an iterator works on it: `for c in s.chars()`, `s.chars().any(...)`, `s.chars().count()`, and so on.
