...
You started with small borrowed-string checks, gave their summary a type, and
combined the rules into a report. The generator reversed the problem: it
constructed an input with known properties and rejected requests it couldn't
satisfy.
...
- Collecting all missing requirements is a different error-handling choice from
  returning after the first failure.
- Precise boundary tests catch mistakes that one "weak" and one "strong" example
  would miss.
