...
- The `Result` constructors `Ok(value)` and `Err(message)` are in
  the prelude, so you can use them without importing anything.
- `f64 == 0.0` is the bounds check. Floating-point comparison has
  plenty of nasty edge cases in general, but exact zero is fine.
