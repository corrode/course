...
Rust's numeric types are familiar, so a quick overview is enough before focusing on what Rust does differently.
...
`parse` returns a `Result`, which makes a failed parse explicit instead of letting it be ignored by accident.
Using `.unwrap_or(0)` is enough for this exercise.
`Result` also supports handling parse failures directly instead of replacing them with a fallback.
