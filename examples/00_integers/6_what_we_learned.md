...
- Overflow isn't silent.
  A plain `+` past a type's maximum panics in debug and wraps in release.
  When overflow is possible, pick the behavior you want: `saturating_add` (clamp), `checked_add` (`Option`), or `wrapping_add` (opt-in wraparound).
