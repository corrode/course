...
- Assigning or passing a non-`Copy` value transfers ownership.
  The old binding is no longer usable.
  `Copy` types (integers, bools, chars, fixed-size arrays of those) are duplicated bit-for-bit instead.
- Borrows let you read or modify a value without taking ownership.
  `&T` is a shared (read-only) borrow; `&mut T` is exclusive.
- The borrow-checker rule: at any time, either any number of `&T` borrows or exactly one `&mut T` borrow, never both.
...
- `clone()` is the explicit escape hatch when you really do need two owners.
  Reach for it after you understand why a borrow won't compile, not before.
