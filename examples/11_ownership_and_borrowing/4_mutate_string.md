...
1. The parameter is `&mut String`, not `&mut str`.
   We need the *owned* `String` because growing it (with `push_str`) may reallocate; a bare string slice has a fixed length.
2. There's no return value.
   The mutation happens through the reference and is visible to the caller after the call returns.
