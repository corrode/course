...
A spreadsheet export arrives with semicolons instead of commas. Implement
`parse_delimited_line(line, delimiter)` without changing the quoting rules. A
delimiter inside quotes is still data, and doubled quotes still mean one literal
quote. Commas become ordinary data when the delimiter is `;`.

If you'd like to adapt your earlier quote parser, copy its body into this
editor; changes in earlier editors don't carry over. You can also work directly
from the starter below. Keep `parse_csv_line` as a thin wrapper that chooses a
comma; don't maintain two parsing loops.
