...
/// to their counts.
///
/// Checking the key yourself with
/// `if contains_key(k) { *map.get_mut(k).unwrap() += 1 } else { map.insert(k, 1) }`
/// works, but it does two lookups. You also can't hold a reference into the map
/// while calling `insert` on it. The standard idiom is
