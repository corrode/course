...
- `HashMap<K, V>` stores key-value pairs and looks them up in (average) constant time.
  ...
  If you need different value types in one map, put them in an enum.
...
- `get` returns `Option<&V>`, never null.
  Combine with `unwrap_or` (or `cloned().unwrap_or(...)` when you need an owned value) to handle the missing case.
- `entry(key).or_insert(default)` is the idiomatic "look up; if missing, insert a default; then return a `&mut V`" pattern.
  It does one lookup instead of two, without needing to call `insert` while holding a reference into the map.
