...
- `push` appends, `pop` removes the last item and returns `Option<T>`, `len` returns the number of items, and `is_empty` tells you whether there are any.
...
- A `for item in &list` loop yields `&T`.
  That's usually what you want; iterating `&mut list` gives `&mut T`, and iterating `list` by value moves the items out.
- A `Vec<String>` is not the same as a `Vec<&str>`.
  Converting between them needs `.to_string()` / `String::from` (one direction) or `.as_str()` (the other).
