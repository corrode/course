...
- [`<[T]>::contains`](https://doc.rust-lang.org/std/primitive.slice.html#method.contains) is the obvious tool, but its signature is `fn contains(&self, x: &T) -> bool`.
...
- [`str::eq`](https://doc.rust-lang.org/std/primitive.str.html#impl-PartialEq%3Cstr%3E-for-String) via `==` is the cleanest way to compare two strings regardless of ownership; no manual `.as_str()` needed.
