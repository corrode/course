...
///
/// Each token is parsed with `?`: the first one that isn't a number short-circuits and returns its `ParseIntError`.
/// The function only parses (no file I/O), so a single error type is enough and there's no need for `Box<dyn Error>`.
