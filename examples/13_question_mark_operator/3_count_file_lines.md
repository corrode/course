...
File I/O returns [`std::io::Error`](https://doc.rust-lang.org/std/io/struct.Error.html); the function signature has to declare it as the error type so `?` is happy passing it through.
...
- [`std::fs::read_to_string`](https://doc.rust-lang.org/std/fs/fn.read_to_string.html) reads the whole file into a `String`.
  Returns `Result<String, io::Error>`, which is exactly what `?` wants.
