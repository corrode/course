...
- Return `"not a valid percentage"` when the text cannot be parsed as a `u8`,
  including negatives and values above `255`.
- Return `"percentage must be between 0 and 100"` when parsing succeeds but the
  value is above `100`.
...
- [`str::parse`](https://doc.rust-lang.org/std/primitive.str.html#method.parse)
  ...
  negative numbers and anything above `255`, so those inputs produce parse
  errors.
