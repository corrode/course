...
- The `Result` constructors `Ok(value)` and `Err(message)` are in the prelude,
  so you can use them without importing anything.
- `divisor == 0.0` detects both positive and negative zero, the failure cases
  for this exercise.
