...
Run the supplied code first: three tests pass, and `test_username_survives_records` stops at `todo!()`.
...
3. Change the return type and body of `find_user_by_id` so the caller can use the username after the records are dropped.
   ...
   The records must still be dropped at the end of their block.
...
All four tests should pass when you're done.
To set the exercise aside, restore the last test's comment markers and `todo!()` rather than removing the test.
