...
A trait method can ship with a default body. Implementors get that
method for free, but any one of them can override it if the default
doesn't fit.
...
1. `PlainLogger` returns the message untouched. It uses both defaults
   as written, so all you have to write is `log`.
2. `TaggedLogger { tag: String }` prepends a tag (e.g. `"auth: ..."`).
   It uses the default `warn`, but *overrides* `error` to swap the
   `[ERROR]` prefix for a louder `[CRITICAL]` prefix.

That asymmetry is the point: implementors take what they want from
the defaults and override only the bits they need to change.

## Useful from the standard library

- The `format!` macro is the workhorse here. Default `warn` builds
  `"[WARN] {msg}"` and hands it back to `self.log`, so whatever
  decoration `log` does (the tag, in `TaggedLogger`'s case) wraps
  the warning prefix.
- Default methods are written *inside* the `trait` block, with a body
  instead of a trailing semicolon. The required methods (the ones
  without a body) are still mandatory; defaults are bonus.
