...
- [`std::matches!`](https://doc.rust-lang.org/std/macro.matches.html) expands to
  a `match` that returns `true` for the given pattern and `false` otherwise. You
  can write it as `matches!(status, HttpStatus::InternalServerError)`.
- [`PartialEq`](https://doc.rust-lang.org/std/cmp/trait.PartialEq.html) enables
  `==`, so the enum's derived implementation also lets you write
