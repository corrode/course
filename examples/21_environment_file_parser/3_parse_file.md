...
Each step is self-contained, so the previous step's `parse_env_line` and `ParseError` are re-declared here with `todo!()` bodies.
Re-implement them (or paste your earlier solution) so this step compiles on its own.
