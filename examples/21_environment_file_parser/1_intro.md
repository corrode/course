...
We'll build a parser for `.env`-style configuration files.
Along the way, you'll split a string only once and write a generic function that parses into the type its caller asks for.
...
The parser you're about to write uses one error type, the custom `ParseError` enum, so `?` propagates it cleanly.
