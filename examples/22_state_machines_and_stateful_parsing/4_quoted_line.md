...
Walk the string character by character with a peekable iterator and keep a small
`in_quotes: bool` flag. When you see `"` while already inside quotes, peek at
the next character. If it's another `"`, push a literal `"` and consume both;
otherwise close the field. Keeping track of which quote you've consumed is the
hard part here. If a test fails, trace its input character by character and
write down `in_quotes` after each step.
...
- [`Iterator::peekable`](https://doc.rust-lang.org/std/iter/trait.Iterator.html#method.peekable)
  wraps the iterator so you can look ahead one character. You need that
  lookahead for the `""` -> `"` rule.
...
- A `match (c, in_quotes)` on the tuple lets you express each state transition
  as a single arm. Add a guard (`if chars.peek() == Some(&'"')`) for the escape
  rule.
