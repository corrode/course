...
Pay more attention to the signature and the test: after the call, the caller can still read and grow its owned `String` because the function only borrowed it.
...
- Rust calls the automatic conversion from `&String` to `&str` a "deref coercion."
  A caller with a `String` can pass `&s` without calling `.as_str()`; the tests cover both an owned `String` and a string literal.
