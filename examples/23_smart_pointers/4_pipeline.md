...
- `Reverse` reverses Unicode scalar values, which can separate combining marks
  from their letters.
- `Append { suffix: String }` appends its owned suffix.

A `Vec<C>` with `C: Command` has just one concrete element type. A
...
- `make_pipeline(suffix: String) -> Vec<Box<dyn Command>>` returns exactly two
  ...
  factory returns.
- `apply_pipeline(commands: &[Box<dyn Command>], input: &str) -> String` passes
  ...
  implementation of `Command`, not just the three supplied types.

For a suffix of `"x"`, the factory's pipeline transforms `"hi"` into `"HIx"`,
