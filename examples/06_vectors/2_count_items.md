...
Notice the parameter is `&Vec<String>`: a borrow, so the caller keeps ownership of the list.

## Useful from the standard library

- [`Vec::len`](https://doc.rust-lang.org/std/vec/struct.Vec.html#method.len)
  returns the number of items as a `usize`.
...
> [!TIP]
> Press `Ctrl/Cmd + Enter` to run the tests without leaving the keyboard.
> The **Run** button does the same thing.
