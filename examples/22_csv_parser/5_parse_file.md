...
Assume each record occupies one line; this parser won't handle newlines inside quoted fields.
...
- [`Iterator::collect`](https://doc.rust-lang.org/std/iter/trait.Iterator.html#method.collect) gathers the parsed rows into a `Vec<Vec<String>>`.
  `parse_csv_line` already returns the headers as a `Vec<String>`.
