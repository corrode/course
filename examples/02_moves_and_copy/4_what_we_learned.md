...
- Moving a `String` transfers its pointer, length, and capacity without copying the heap buffer.
  To copy the buffer too, call `.clone()`.
...
- Borrowing lets a function use a value without taking ownership from its caller.
