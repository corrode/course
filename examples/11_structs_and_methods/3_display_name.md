...
- [`format!`](https://doc.rust-lang.org/std/macro.format.html)
  builds a new `String` from a template and arguments. Same syntax
  as `println!`, but returns the string instead of printing it.
- Field access uses dot notation: `self.name`, `self.email`. Inside
  a `format!` template you can interpolate them inline:
