...
- The `+= 1` operator updates a numeric field in place; the same
  works through `self`. No `Cell` or special-casing required.
- Plain assignment (`self.is_verified = true`) is enough for the
  bool. There's no separate "setter" syntax in Rust; methods on
  `&mut self` just assign.
- [`u32::checked_add`](https://doc.rust-lang.org/std/primitive.u32.html#method.checked_add)
  is the safe-overflow alternative if you're worried about
  wrapping. Not needed here.
