...
- `word.chars().count()` measures string length in characters,
  which is usually what you want. `str::len` returns *bytes*, and
  the two differ the moment you hit a non-ASCII character.
...
Then in **chapter 17 ("Word Frequencies")** we'll go further and
ask not just *how many* words a text contains, but *which* words
appear and *how often* each one shows up.
