...
- **`stray_semicolon`** drove home the difference between an expression
  and a statement: one trailing `;` is the line between "return this
  value" and "return `()`".
- **`sum_to`** built a value with recursion, where each call's answer
...
value. Everything else in the chapter is a consequence of that one rule.
