...
doesn't compile, because function parameters are immutable bindings by default
(like `let`), so reassigning `value` is rejected. The fix is one keyword in one
place. Read the error, it points right at it.
...
`Copy`, so the function received its own copy to mutate. You'll see what changes
for non-`Copy` types like `String` when ownership and borrowing come up later.
