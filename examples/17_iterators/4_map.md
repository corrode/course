...
- [`Vec::into_iter`](https://doc.rust-lang.org/std/vec/struct.Vec.html#method.into_iter) consumes the vec and yields owned items.
  ...
  `to_lowercase` only borrows each string, so ownership is not required for the conversion itself.
...
- [`str::to_lowercase`](https://doc.rust-lang.org/std/primitive.str.html#method.to_lowercase) returns a fresh `String` with characters converted to lowercase.
  Lowercasing is not the same as Unicode case folding for caseless comparison.
