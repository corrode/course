...
Same as the previous step, but the input is a `&[&str]` (a borrowed slice of borrowed strings), so the iterator yields `&&str`.
We sidestep that double-reference by returning owned `String`s; the lesson here is iterators, not lifetimes.

To go from `&&str` to `String`, reach for [`str::to_string`].
...
- [`str::to_string`](https://doc.rust-lang.org/std/primitive.str.html#method.to_string) is the easy `&str` -> `String` call.
