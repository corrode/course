...
///
/// Now you need to transform every element instead of reducing the sequence to
/// one value. Start with `vec.into_iter()`, apply a closure to each item with
/// an adapter, then build a new `Vec` with `collect()`. See:
