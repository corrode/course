...
- Fallible `sum` stops at the first `Err` and returns it as a value.
  Unlike `?`, it doesn't return from the surrounding function.
- `next` and `take` can consume only part of a pipeline.
  With `by_ref`, you can resume the same iterator afterward; already-consumed items don't run again.
